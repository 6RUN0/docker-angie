...
| Tag | Meaning |
| --- | --- |
| `1.12.1-build1-alpine`, `…-debian` | Immutable — exact Angie version and packaging. **Pin this.** |
| `1.12.1-alpine` | Latest build of that Angie patch. |
...
| `*-unprivileged` | Rootless variant; suffixes every tag above except `latest` (e.g. `alpine-unprivileged`, `1.12.1-build1-alpine-unprivileged`). |

Floating tags move only for stable Angie releases; a prerelease Angie version
(e.g. `1.12.1-rc1`) publishes its immutable `…-build<N>` tag only.
