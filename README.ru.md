...
| Тег | Значение |
| --- | --- |
| `1.11.8-build3-alpine`, `…-debian` | Неизменяемый — точная версия Angie и упаковка. **Фиксируйте его.** |
...
| `*-unprivileged` | Rootless-вариант; суффикс ко всем тегам выше, кроме `latest` (например, `alpine-unprivileged`, `1.11.8-build3-alpine-unprivileged`). |
