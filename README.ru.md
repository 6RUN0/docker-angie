...
| Тег | Значение |
| --- | --- |
| `1.12.1-build1-alpine`, `…-debian` | Неизменяемый — точная версия Angie и упаковка. **Фиксируйте его.** |
| `1.12.1-alpine` | Последняя сборка этого патча Angie. |
...
| `*-unprivileged` | Rootless-вариант; суффикс ко всем тегам выше, кроме `latest` (например, `alpine-unprivileged`, `1.12.1-build1-alpine-unprivileged`). |

Плавающие теги двигаются только для стабильных релизов Angie; prerelease-версия
Angie (например, `1.12.1-rc1`) публикует лишь свой неизменяемый тег `…-build<N>`.
