...
выключите access_log и его формат вместе (один вызов `angie-ctl dis` снимает оба
симлинка, затем валидирует один раз):
...
Сам geoip2-map осиротевает так же — оставшийся от прошлого запуска
`geoip2 <путь>` валит `angie -t` с `MMDB_open(...) failed`, когда базы данных
уже нет. Entrypoint снимает map и его модуль при старте, так что
пересоздание контейнера лечит и это.

**Общее правило.** Тумблеры фич декларативны: на каждом старте entrypoint
сбрасывает управляемые им сниппеты и модули, затем включает заново только то,
что задано текущим окружением `ANGIE_*`. Снятие переменной выключает её фичу на
следующем старте (перезапустите или пересоздайте контейнер, чтобы entrypoint
отработал заново), даже на персистентном томе `/etc/angie`. Наслаивайте свою
конфигурацию через том `/etc/angie/custom`, а не включайте поставляемые сниппеты
вручную — включённый вручную сниппет будет снят на следующем старте. Например,
отключение access_log по умолчанию (`ANGIE_LOG_LOGFMT=no`) без выбора другого
формата оставляет конфиг вовсе без глобального `access_log` — это декларативный
результат, а не ошибка (`angie -t` при этом проходит). Единственное
исключение — автоподбор `worker_processes`
(`ANGIE_ENTRYPOINT_WORKER_PROCESSES_AUTOTUNE`): он правит `angie.conf` на месте
за одноразовым сентинелом, поэтому подобранное на прошлом запуске значение
сохраняется, пока вы не сбросите файл или не пересоздадите том (это безвредное
значение производительности, `angie -t` оно не ломает).
